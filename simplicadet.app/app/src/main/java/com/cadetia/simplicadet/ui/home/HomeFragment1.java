...
public class HomeFragment1 extends Fragment implements LearningPathAdapter.OnLearningPathClickListener, JournalAdapter.OnJournalClickListener {
    ...
    private void markNodeAsCompleted(int position) {
        if (position >= 0 && position < learningPathList.size() && currentLearningPath != null) {
            ...
            learningPathAdapter.notifyDataSetChanged(); updateLearningPathHeader();
    ...
    private void showRedeemDialog(int score, int correct, int total, float time) {
        if (score > 0) {
            ...
            sheet.setArguments(b); sheet.show(requireActivity().getSupportFragmentManager(), sheet.getTag());
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == LEARNING_PATH_REQUEST && resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (position != -1 && position < learningPathList.size()) {
                LearningPathModel completedNode = learningPathList.get(position);
                if (completedNode.getType() == 0) {
                    int score = data.getIntExtra("totalScore", 0); int correct = data.getIntExtra("correctAnswers", 0);
                    int total = data.getIntExtra("totalQuestions", 0); long time = data.getLongExtra("totalTime", 0L);
                    handler.postDelayed(() -> showRedeemDialog(score, correct, total, (float) time / 1000), 1000);
                    DbQuery.updateTotalScore(userEmail, score, new MyCompleteListener() { @Override public void onSucces() {} @Override public void onFailure() {} });
                }
                if (!completedNode.isCompleted()) markNodeAsCompleted(position);
            }
        }
    }

    private void retrieveUserData() { SharedPreferences sp = getActivity().getSharedPreferences("UserData", MODE_PRIVATE); userEmail = sp.getString("userEmail", ""); }
}